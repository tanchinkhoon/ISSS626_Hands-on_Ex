...
tm_shape(kbb_sf) +                                                  # add study area polygon layer
  ...
  tm_credits(
    "Forest Fire Points in 2023 (Kep. Bangka Belitung)",
...
Thereafter, we will reproduce month-specific density maps like the professor’s July–December example.
...
```{r}
# Refit STKDE using the professor's recommended bandwidths 
kde_yday <- spattemp.density(
  fire_yday_owin,     # the DayofYear ppp constrained by the study window
  h      = 9000,      # spatial bandwidth in meters (per professor's improved selection)
