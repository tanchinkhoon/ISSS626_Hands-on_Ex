...
### 10.1 G-function Analysis

The K-function measures the number of neighbours within distance r of each event and compares it against CSR. It detects clustering or dispersion at multiple spatial scales.

```{r}

# K for Tampines (reusing Sect. 9 PPP)
K_tm      <- Kest(rest_ppp_tampines_km, correction = "Ripley")
K_tm.csr  <- envelope(rest_ppp_tampines_km, Kest, nsim = 99, rank = 1, glocal = TRUE)
...
K_pg.csr  <- envelope(rest_ppp_punggol_km,  Kest, nsim = 99, rank = 1, glocal = TRUE)
...
K_tm <- Kest(rest_ppp_tampines, correction = "Ripley")
K_tm.csr <- envelope(rest_ppp_tampines, Kest, nsim = 99, rank = 1, glocal = TRUE)

plot(K_tm.csr, . - r ~ r,
     xlab = "Distance (m)", ylab = "K(d)-r",
     main = "K-function with CSR envelope (Tampines, Jan–Jun 2025)")

# K-function for Punggol
K_pg <- Kest(rest_ppp_punggol, correction = "Ripley")
K_pg.csr <- envelope(rest_ppp_punggol, Kest, nsim = 99, rank = 1, glocal = TRUE)
...
G_rest <- Gest(biz_56111_ppp, correction = "border")
G_rest.csr <- envelope(biz_56111_ppp, Gest, nsim = 999)

plot(G_rest.csr, main = "G-function with CSR envelope (Restaurants, Jan–Jun 2025)")


```

### 10.2 F-function Analysis

```{r}

# L for Tampines
L_tm      <- Lest(rest_ppp_tampines_km, correction = "Ripley")
L_tm.csr  <- envelope(rest_ppp_tampines_km, Lest, nsim = 99, rank = 1, glocal = TRUE)
...
L_pg.csr  <- envelope(rest_ppp_punggol_km, Lest, nsim = 99, rank = 1, glocal = TRUE)
...
p_tm <- ggplot(L_tm_df, aes(r, obs - r)) +
  ...
  geom_rug(data = L_tm_df[L_tm_df$obs > L_tm_df$hi,], sides = "b") +  # significant clustering
  geom_rug(data = L_tm_df[L_tm_df$obs < L_tm_df$lo,], sides = "b") +  # significant dispersion
...
p_pg <- ggplot(L_pg_df, aes(r, obs - r)) +
  ...
  geom_line(aes(y = theo - r), linetype = "dashed", colour = "red") +
  geom_ribbon(aes(ymin = lo - r, ymax = hi - r), alpha = 0.2) +
  geom_rug(data = L_pg_df[L_pg_df$obs > L_pg_df$hi,], sides = "b") +
  geom_rug(data = L_pg_df[L_pg_df$obs < L_pg_df$lo,], sides = "b") +
  xlab("Distance r (m)") + ylab("L(d) - r") +
...
### 10.4 Spatio-temporal Second-order Analysis (STIKhat) — STRICT REUSE
...
# Reuse-only guards
stopifnot(exists("biz_56111_sf"),
          exists("rest_ppp_tampines_km"),
          exists("rest_ppp_punggol_km"),
          "MONTH_NUM" %in% names(biz_56111_sf))

# 1) Subset global sf by the *existing* PPP windows (no new polygons)
coords_all <- st_coordinates(biz_56111_sf)

inside_tm <- inside.owin(coords_all[,1], coords_all[,2], rest_ppp_tampines_km$window)
inside_pg <- inside.owin(coords_all[,1], coords_all[,2], rest_ppp_punggol_km$window)

rest_sf_tampines <- biz_56111_sf[inside_tm, ]
rest_sf_punggol  <- biz_56111_sf[inside_pg, ]

# 2) Drop rows with missing geometry or time mark (required by STIKhat)
rest_sf_tampines <- rest_sf_tampines[!is.na(rest_sf_tampines$MONTH_NUM) & !st_is_empty(rest_sf_tampines), ]
rest_sf_punggol  <- rest_sf_punggol[ !is.na(rest_sf_punggol$MONTH_NUM)  & !st_is_empty(rest_sf_punggol), ]

# Helper to run STIKhat safely on an sf subset
run_stik <- function(sf_subset, title_lbl) {
  if (nrow(sf_subset) < 5L) {           # small-n guard; adjust if you wish
    message(sprintf("[STIKhat skipped] %s has < 5 points after filtering.", title_lbl))
    return(invisible(NULL))
  }
  coords <- st_coordinates(sf_subset)
  # Ensure time is integer (Jan–Jun labels should already be 1..6 in your pipeline)
  tvec <- as.integer(sf_subset$MONTH_NUM)
  # Filter any remaining anomalies
  good  <- is.finite(coords[,1]) & is.finite(coords[,2]) & is.finite(tvec)
  coords <- coords[good, , drop = FALSE]
  tvec   <- tvec[good]
  if (length(tvec) < 5L) {
    message(sprintf("[STIKhat skipped] %s has < 5 valid events after QC.", title_lbl))
    return(invisible(NULL))
  }
  df <- data.frame(x = coords[,1], y = coords[,2], t = tvec)
  stpp_obj <- as.3dpoints(df)
  stik_obj <- STIKhat(stpp_obj)
  plotK(stik_obj); title(main = sprintf("STIKhat contours — %s (Jan–Jun 2025)", title_lbl))
  invisible(stik_obj)
}

# 3) Run for Tampines and Punggol (reused objects only)
tm_stik <- run_stik(rest_sf_tampines, "Tampines")
pg_stik <- run_stik(rest_sf_punggol,  "Punggol")
```
