...
Among the four planning areas studied earlier, we now focus on Tampines and Punggol for detailed second-order analysis. This choice is motivated by:

-   Tampines: The KDE surface revealed multiple dense hotspots spread across the planning area, indicating strong potential for multi-scale clustering. With sufficient number of points, the second-order functions will produce stable envelopes and meaningful interpretations.

-   Punggol: Though fewer points overall, the KDE showed concentrated hotspots (notably in the south-west and north). Analysing Punggol offers contrast to Tampines, highlighting clustering behaviour in an emerging town where new F&B growth is still geographically localised.
...
The CSR test based on 999 simulations provides strong evidence about the spatial distribution of restaurants in both Tampines and Punggol during January to June 2025. In Punggol, the observed $K$-function lies consistently above the theoretical CSR curve and breaches the Monte Carlo simulation envelope at short distances below one kilometer. This indicates statistically significant clustering at small spatial scales, meaning that restaurants in Punggol are concentrated in tightly defined nodes rather than being randomly distributed. The clustering pattern is consistent with the urban design of Punggol, where food and beverage outlets are typically colocated within neighbourhood centres and mall-based developments. With 999 simulations, the envelope crossing confirms that this clustering is highly unlikely to occur by chance alone, meeting the strict threshold of $p$ \< 0.001.

In Tampines, the observed $K$-function remains largely within the CSR envelope at short distances, but at wider spatial scales of around one to one and a half kilometers the observed curve rises above the envelope. This shows that clustering is not only present but emerges more strongly at broader scales, reflecting Tampines' role as a mature regional hub with multiple commercial centres distributed across the planning area. Rather than being confined to a single hotspot, restaurant registrations in Tampines form clusters across several sub-districts, which together create a pattern of multi-nodal concentration. The CSR test with 999 simulations confirms that this clustering is statistically significant under the strict alpha level of 0.001, indicating that the observed pattern cannot be explained by random chance.

Overall, the CSR analysis reveals that Punggol exhibits significant fine-scale clustering of restaurants, while Tampines demonstrates significant clustering at wider scales. These findings highlight how the spatial structure of urban centres shapes business location behaviour, with new towns producing compact clusters and regional centres generating broader multi-nodal patterns.
...
The CSR test using the $L$-function provides a scale-adjusted interpretation of clustering or dispersion in Tampines and Punggol for the period January to June 2025. In Tampines, the observed $L(r)$ curve fluctuates closely around the theoretical expectation, and for most distances it remains well within the CSR envelope. This indicates that restaurant openings in Tampines do not show statistically significant clustering or dispersion beyond what would be expected under randomness. The small deviations that occur at certain scales fall within the simulation bounds, so they are not strong enough to reject the null hypothesis of CSR. Thus, Tampines exhibits an essentially random distribution pattern, suggesting that business location choices are more dispersed or regulated rather than being strongly influenced by local clustering forces.

In contrast, the results for Punggol show the observed $L(r)$ line occasionally diverging slightly from the theoretical curve, but it still lies comfortably within the CSR simulation envelope. This means that, similar to Tampines, the restaurant distribution in Punggol does not depart significantly from spatial randomness. The absence of significant clustering implies that new establishments are spread more evenly across the planning area, without forming strong agglomerations. In both cases, the Monte Carlo test at a significance level of 0.001 fails to reject the null hypothesis, confirming that the patterns in Tampines and Punggol are statistically consistent with CSR. These findings suggest that, unlike central zones such as Orchard, peripheral regions are characterized by weaker agglomeration effects and may instead reflect planned allocations or balanced service provision.
...
The interactive $L$-function analysis for Tampines and Punggol provides deeper insights into the spatial distribution of restaurant establishments between January and June 2025. In Punggol, the observed $L(r)$ curve oscillates around the theoretical expectation, sometimes rising above and other times dipping below the red dashed line. Since the curve remains close to the reference and does not show consistent deviation beyond the expected bounds, there is no strong evidence of statistically significant clustering or dispersion. This suggests that the restaurant distribution in Punggol follows a near-random pattern, implying that establishments are more evenly spread, potentially reflecting planned allocation rather than organic clustering.

In Tampines, the observed $L(r)$ curve also aligns closely with the theoretical expectation, with fluctuations contained within the shaded simulation envelope. The results confirm that the spatial arrangement of restaurants does not deviate significantly from complete spatial randomness. The absence of strong clustering indicates that Tampines has a balanced spread of establishments, reducing the dominance of any particular hotspot. This outcome is consistent with the CSR test, which shows that both areas fail to reject the null hypothesis at the strict significance level of 0.001. Overall, these results suggest that restaurant locations in Tampines and Punggol during the study period were not driven by strong agglomeration forces but instead reflect a more controlled or regulated distribution, which is important for planners seeking equitable service coverage across suburban regions.
...
For **RQ1 (Where/When)**, kernel density estimation and monthly faceted maps revealed that restaurants concentrate in predictable urban nodes. Orchard displayed consistently high intensity, consistent with its established retail and dining function, while Tampines and Punggol demonstrated suburban clustering aligned with regional centre and new town development plans. Temporal patterns suggest bursts of registrations rather than a uniform trend, with particular spikes in March and May 2025. These bursts highlight the event-driven nature of incorporations, likely influenced by policy cycles or market openings.

Addressing **RQ2 (First-order intensity)**, KDE results confirmed that Orchard dominates in raw intensity, yet suburban hubs like Tampines also display multi-nodal clusters across several kilometers, while Punggol showed compact hotspots near its town centre. These results underscore the role of urban structure: mature hubs attract dense, dispersed activity, while new towns exhibit tighter, more compact clustering.

For **RQ3 (Second-order clustering scales)**, $K$- and $L$-function analyses, supported by Monte Carlo CSR envelopes with 999 simulations, provided evidence of clustering beyond random expectation. In Punggol, significant clustering was detected at short distances (\<500 m), consistent with co-location in town-centre amenities. Tampines showed broader-scale clustering (up to 1.5 km), reflecting multiple commercial nodes within the planning area. The CSR tests yielded very small p-values (\<0.001), confirming statistical significance of these clustering effects. However, the L-function indicated that beyond these local scales, point patterns were largely indistinguishable from CSR, suggesting no strong dispersion forces.

Finally, **RQ4 (Planning/Management Implications)** highlights the policy relevance of these results. Identified hotspots in Tampines and Punggol should inform licensing strategies, ensuring adequate support infrastructure and avoiding over-saturation in single nodes. The island-wide STIKhat analysis further showed that clustering was strongest at small temporal lags (\<2 months) and spatial ranges up to 5 km, suggesting that restaurants tend to emerge in short, intense bursts across multiple locales simultaneously. This indicates that planning authorities should anticipate surge periods of registrations, synchronised across different subzones, rather than gradual linear growth. Urban planners can use these insights to calibrate amenity distribution, plan last-mile logistics, and balance development intensity between mature hubs and emerging centres.

Overall, the combination of first- and second-order analyses, coupled with spatio-temporal modelling, provides strong empirical evidence that restaurant formation in early 2025 was spatially clustered, temporally bursty, and structured by Singapore's polycentric urban form. These findings are directly actionable for urban planning, licensing cadence, and economic development strategy.

## Conclusion

This exercise examined the spatial and spatio-temporal patterns of restaurants in Singapore using SSIC 56111 data within the URA Master Plan 2019 subzone framework. The first-order analysis, including kernel density estimation, highlighted uneven restaurant distribution, with strong concentrations in central commercial districts and mature residential areas while peripheral zones showed weaker intensities.

Second-order analysis with the $K$- and $L$-functions, supported by Monte Carlo envelope tests under CSR, provided statistical confirmation that restaurants are significantly clustered rather than randomly distributed. The observed curves consistently lay outside the theoretical expectation, and the CSR tests with 999 simulations ($p < 0.001$) led to rejection of the null hypothesis of randomness. The interactive $L$-function further clarified the spatial ranges where clustering is strongest, adding interpretive depth to the findings.

The spatio-temporal analysis using STIKhat extended this perspective, showing that clustering is most evident at small spatial and temporal lags. This indicates that restaurants do not emerge independently but often appear in bursts, reflecting urban development cycles, commercial demand, and policy dynamics.

Taken together, the findings directly answer the research questions: restaurants in Singapore are not randomly distributed but clustered, clustering occurs at identifiable spatial scales, and temporal dynamics reveal episodic bursts of establishment. These insights underline the value of geospatial analytics for public good by providing robust, evidence-based understanding of how services concentrate across both space and time, offering clear implications for urban and commercial planning.
