...
# Shared layout template for consistency and professional presentation
...
# Verify that all three objects exist and share the same CRS and field names
# sf::st_geometry_type(map_am)
# sf::st_crs(map_am)
# names(map_am)
...
# 3) Standardised Moran scatter (points + black OLS; dashed mean lines), per professor
library(ggplot2)
...
#### 10.4.4.3 Preparing LISA Map Classes ($p$ < 0.05)
...
# not "sf" — use "geometry"
sf_grid <- attr(st_cube, "geometry")   # this is an sf POLYGON/MULTIPOLYGON
stopifnot(inherits(sf_grid, "sf"))
...
nb_st_s  <- spdep::poly2nb(sf_grid, queen = TRUE)
nb_st_s  <- spdep::include.self(nb_st_s)                     # Gi* needs self
lw_st_s  <- spdep::nb2listw(nb_st_s, style = "W", zero.policy = TRUE)
...
# trips_long: HEX_ID, window (factor: Morning/Afternoon/Weekend), Trips (numeric)

gi_by_window <- sf_grid |>
  dplyr::select(HEX_ID, geometry) |>
  dplyr::left_join(trips_long, by = "HEX_ID") |>
  sf::st_as_sf() |>
  ...
  dplyr::mutate(
    gistar = sfdep::local_gstar_perm(
      x   = as.numeric(Trips),
      nb  = nb_st_s,
      wt  = lw_st_s$weights,  # <- the weights list
      nsim = 999,
      alternative = "two.sided",
      zero.policy = TRUE
    )
  ) |>
  tidyr::unnest(gistar)   # adds columns like Gi, p_sim, etc.
