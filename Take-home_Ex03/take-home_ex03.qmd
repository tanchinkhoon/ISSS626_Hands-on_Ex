...
**RQ1**: Which structural attributes of a flat such as floor level, age, and remaining lease are significantly associated with resale price during the study window
...
**RQ4**: Does the Geographically Weighted model (GWmodel) materially improve fit and residual behaviour relative to a global Multiple Linear Regression (MLR) specification
...
The adaptive GWR output now attaches local coefficients, standard errors, t values, fitted prices, and residuals to every HDB. This enables mapping spatial variation in effects, identifying neighbourhoods where specific predictors matter most, and diagnosing where the global hedonic model under or over estimates resale prices.
...
Adaptive GWR fitted prices range from about 396,622 to 1,485,595. Median predicted price about 628,332 and mean about 672,147 indicate a slightly right skewed distribution with relatively few high value HDBs pulling the average above the middle of the market.
...
# --- 2) Keep only HDB points that fall inside Tampines -----------------------
# Our GWR results were attached to `analysis.sf.adaptive` and include `Local_R2`.
...
```
The updated Local R² results for Tampines show that the adaptive GWR still fits very well but with more variation than in the smaller earlier sample. Across 712 HDBs, Local R² ranges from about 0.685 to 0.965 with a median around 0.876 and upper quartile near 0.951.

On the map, darker blue clusters in the north west and south east indicate locations where the model explains more than 0.95 of local price variation. Here the hedonic predictors such as floor level, age and accessibility capture HDB resale prices extremely well. Lighter areas in the central belt correspond to Local R² around 0.75 to 0.85, meaning that prices there depend more on factors not included in the model, for example project specific facilities or micro environment qualities.
