...
FREEHOLD,
data = condo_resale.sf,
approach = "AIC", # Chnage from CV to AIC
...
#### 13.9.2.1 Computing the adaptive bandwidth (AIC; K nearest neighbors)
...
FAMILY_FRIENDLY + FREEHOLD,
data = condo_resale.sf,
approach = "AIC", # Change from CV to AIC approach
kernel = "gaussian",
...
## Selecting Tampines planning areas 
...
# Your GWR results were attached to `condo_resale.sf.adaptive` and include `Local_R2`.
...
### Insights and intepretation:

The Geographically Weighted Regression (GWR) model exhibits an exceptionally strong local fit for the Tampines planning area. Among 76 condominium resale observations, the local coefficient of determination (R²) ranges from 0.894 to 0.972, with a median of 0.965 and a mean of 0.955. These values indicate that approximately 95–97 percent of the variation in resale prices within Tampines is explained by the explanatory variables used in the model, such as floor area, building age, and proximity to amenities or transport nodes. Compared with the global OLS R² of 0.65, the local model captures spatial heterogeneity far more effectively, revealing that relationships between housing price and predictors vary significantly across Singapore. The narrow interquartile range (0.955–0.971) further implies high model stability and uniform explanatory power across all Tampines subzones. This consistency suggests that the selected variables are highly relevant and that the urban environment in Tampines is relatively homogeneous, characterised by systematic town planning and similar access to facilities. The strong local R² values also confirm that GWR is a suitable model for capturing spatially varying relationships in mature, well-developed residential areas, offering reliable insights into how local factors influence property values across different neighbourhood contexts.
